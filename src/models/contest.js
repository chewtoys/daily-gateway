...
const select = () =>
  db.select('start_at', 'end_at').from(table);

const getOngoingContest = (now = new Date()) =>
  select()
    .where('start_at', '<=', now)
...
const getUpcomingContest = (now = new Date()) =>
  select()
    .where('start_at', '>', now)
    .andWhere('end_at', '>', now)
...
const referralCounts = (startAt, endAt) =>
  db
...
const getContestLeaderboard = (startAt, endAt) =>
  db
...
const getUserRank = (startAt, endAt, userId) =>
  db
    .select(
      'points',
